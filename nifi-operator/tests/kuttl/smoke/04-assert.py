...
cluster = requests.get(url, headers=headers, verify='tests/kuttl/smoke/cacert.pem')
...
# Check which node is the coordinator and store for later test steps
cluster_dict = json.loads(cluster.content.decode('utf-8'))
controller = [x['address'].split('.')[0] for x in cluster_dict['cluster']['nodes'] if 'Cluster Coordinator' in x['roles']]

controller_file = open("tests/kuttl/smoke/controller", "w")
controller_file.write(controller[0])
